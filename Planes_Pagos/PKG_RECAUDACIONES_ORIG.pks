...
------------------------------------------------------------------
   ...
   FUNCTION recp_0450(p_incorporador PKG_RECAUDACIONES.vincorporacionrec)
      RETURN VARCHAR2;
     ...
     -- FUNCTION recp_0450 RETURN VARCHAR2 AS
      CURSOR c_recaudaciones_externas(
         ...
         SELECT   rep_id, rep_registro
            ...
            WHERE rep_fec_baja IS NULL AND rep_ere_codigo = p_ente AND rep_fecha = p_fecha
         ORDER BY rep_id;
...
--      p_incorporador.p_fecha := TO_DATE('31/12/2019','DD/MM/RRRR');
--      p_incorporador.v_usuario := 'SROMERO';
      ...
      EXCEPTION
         WHEN NO_DATA_FOUND THEN
            ...
            RETURN l_retorno;
         WHEN OTHERS THEN
            l_retorno := 'En optención de numero de remesa de OMS. Error: ' || SQLERRM;
            ROLLBACK;
            RETURN l_retorno;
      END;
      ...
      FOR r_recaudaciones_externas IN c_recaudaciones_externas(p_incorporador.p_ente, p_incorporador.p_fecha) LOOP
         ------------------------------------------------------------------------------
         --------- Esta funcion realiza  un TEST por cada registro que se procesa -----
         ------------------------------------------------------------------------------
         --IF RevisaAcreditaciones > 0 THEN
         --   ROLLBACK;
         --   RETURN('VERIFCAR LOG DE RECAUDACION- ACREDITACION SIN REGISTRAR. REP_ID='||TO_CHAR(r_recaudaciones_externas.Rep_id));
         --END IF;
         -------------------------------------------------------------------------------
         ...
         l_codigo_barra :=
               ('00' || SUBSTR(r_recaudaciones_externas.rep_registro, 1, 3))
            || SUBSTR(r_recaudaciones_externas.rep_registro, 26, 55);
